...
/**
 * Base cache for feed, e.g. user feed, category feed
 */
public class BaseFeedCache {

    private String mTag = "BaseFeedCache";
    private long mStaleIntervalMs = 3 * DateUtils.MINUTE_IN_MILLIS;

    private ImmutableList<FeedPost> mCachedFeed;
    private long mLastUpdatedMs;

    BaseFeedCache(String tag, long staleIntervalMs) {
        mCachedFeed = ImmutableList.of();
        mTag = tag;
        mStaleIntervalMs = staleIntervalMs;
    }

    public boolean isUpToDate() {
        return System.currentTimeMillis() - mLastUpdatedMs < mStaleIntervalMs;
    }
    ...
    public synchronized ImmutableList<FeedPost> updateCacheFromDb(
        ...
        Log.d(mTag, "In updateCacheFromDb, mCachedFeed=" + mCachedFeed.size() +
        ...
        for (FeedPost post: dbFeedPosts) {
            if (hasFoundLast) {
                ...
                Log.w(mTag, "Warning, there is a hole between memory cache and dbFeedPosts.");
    ...
    private void clearFeedPostTableOnThread() {
        Log.d(mTag, "In clearFeedPostTableOnThread");
        ExecutorUtil.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.e(mTag, "clearFeedPostTable failed.", ex);
                }
            }
        });
    }
    ...
    public synchronized ImmutableList<FeedPost> updateCacheFromServer(
        ...
        Log.d(mTag, "In updateCacheFromServer, mCachedFeed=" + mCachedFeed.size() +
        ...
        if (feedFromServer.isEmpty()) {
            Log.d(mTag, "updateCacheFromServer, empty feed from server");
            mCachedFeed = ImmutableList.of();
            return mCachedFeed;
        }
        ...
        for (FeedPost post: mCachedFeed) {
            if (hasFoundLast) {
                ...
                Log.w(mTag, "Warning, there is a hole between memory cache and feedFromServer.");
                mLastUpdatedMs = System.currentTimeMillis();
                mCachedFeed = feedFromServer;
                return mCachedFeed;
            }
    ...
    public synchronized ImmutableList<FeedPost> updateCacheFromServer(
            long timeSince,
            ImmutableList<FeedPost> feedFromServer) {
        Log.d(mTag, "In updateCacheFromServer for load-more, mCachedFeed=" + mCachedFeed.size() +
                ", feedFromServer=" + feedFromServer.size() +
                ", timeSince=" + timeSince);
        if (mCachedFeed.isEmpty()) {
            mCachedFeed = feedFromServer;
            Log.e(mTag, "mCachedFeed became empty after loading more", new RuntimeException());
            return mCachedFeed;
        }
        if (feedFromServer.isEmpty()) {
            Log.d(mTag, "updateCacheFromServer for load-more, empty feed from server");
            return mCachedFeed;
        }
        ...
        if (timeSince != firstFromServer.timeToSort) {
            Log.e(mTag, "Inconsistent timeSince after loading more, " +
                    timeSince + " vs " +
        ...
        if (lastInMemory.timeToSort != firstFromServer.timeToSort) {
            Log.e(mTag, "Unmatched timeSince after loading more, " +
    ...
    public synchronized void incrementCommentCount(String postId) {
        Log.d(mTag, "In incrementCommentCount for " + postId);
        ImmutableList.Builder<FeedPost> builder = ImmutableList.builder();
        for (FeedPost post: mCachedFeed) {
    ...
    public synchronized void changeLikeCountAndIsLiked(String postId, boolean isLiked) {
        Log.d(mTag, "In changeLikeCount for " + postId + ", isLiked=" + isLiked);
        ImmutableList.Builder<FeedPost> builder = ImmutableList.builder();
        for (FeedPost post: mCachedFeed) {
            if (!post.postId.equals(postId)) {
                builder.add(post);
